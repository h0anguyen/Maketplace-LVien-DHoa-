document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchDistricts(provinceCode) {
    ...
    if (provinceCode) {
      const response = await fetch(
        `/checkout/api/districts?provinceCode=${provinceCode}`
      );
  ...
  async function fetchWards(districtCode) {
    ...
    if (districtCode) {
      const response = await fetch(
        `/checkout/api/wards?districtCode=${districtCode}`
      );
...
function loadMoreProducts() {
  ...
  if (url === "/") {
    endpoint = `/api/products?page=${currentPage}&limit=${productsPerPage}`;
  } else {
    console.error("URL không được hỗ trợ");
  }
...
function createProductElement(product) {
  ...
  col.className =
    "col-md-3 col-lg-2 shadow pt-2 mt-1 mb-1 mt-1 mb-1 ml-3 mr-3 col-xs-3";

  col.innerHTML = `
    <div class="product text-center fancy-product">
      <figure class="product-media">
        <a class="product-image-link" href="/product/${product.id}">
          <img class="product-image hover-effect" src="data:image/png;base64,${
            product.mainImage
